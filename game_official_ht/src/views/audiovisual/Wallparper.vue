...
export default {
  ...
  methods: {
    async getAudiovisualList () {
      const data = Qs.stringify({ audioVisualFeastType: 1, pageNum: this.currentPage4, pageSize: this.pageSize })
    ...
    publishHandle () {
      this.$router.push({ path: '/audiovisual/add', query: { path: '/wallparper' } })
    },
    editHandle (id) {
    // this.preImgDialogVisible = true
      this.$router.push({ path: '/editaudiovisual', query: { id, path: '/wallparper' } })
    },
    ...
    recoverHandle (audioVisualFeastId) {
      this.$confirm('是否继续该操作？', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
