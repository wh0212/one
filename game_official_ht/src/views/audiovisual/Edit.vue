          ...
          <div>
            <el-form :model="cardRuleForm"  ref="ruleForm" label-width="100px" class="demo-ruleForm">
              <el-form-item label="视听盛宴类型" prop="audioVisualFeastType">
                  <!-- <el-select v-model="cardRuleForm.audioVisualFeastType" placeholder="视听盛宴类型">
...
export default {
  ...
  methods: {
    async getCurrentList () {
      const data = Qs.stringify({ audioVisualFeastId: this.$route.query.id })
      const { data: res } = await this.$http.post('/audioVisualFeast/findAudioVisualFeast', data)
      console.log(res)
      if (res.status !== 200) return this.$message.error(res.msg)
      if (res.data.items[0].audioVisualFeastType === 0) {
      ...
      this.cardImage = res.data.items[0].audioVisualFeastUrl.split(',')
      const arr = res.data.items[0].audioVisualFeastUrl.split(',')
      arr.map((item) => {
        const obj = { url: item }
    ...
    async submitFormHandle (formName) {
      if (this.path === '/wallparper') {
        return this.subimtWallparper()
      }
      if (this.path === '/audiovisual') {
        return this.subimtVideo()
      }
    },
    async subimtWallparper () {
      const info = {
        audioVisualFeastHead: this.cardRuleForm.audioVisualFeastHead,
        audioVisualFeastId: this.cardRuleForm.audioVisualFeastId,
        audioVisualFeastStatus: 1,
        audioVisualFeastUrl: this.cardImage.join(',')
      }
      console.log('subimtWallparper', info)
    ...
    async subimtVideo () {
      const info = {
        ...
        audioVisualFeastId: this.cardRuleForm.audioVisualFeastId,
        audioVisualFeastStatus: 1,
        audioVisualFeastUrl: this.cardRuleForm.audioVisualFeastUrl
      }
      console.log('subimtVideo', info)
      const data = Qs.stringify(info)
      console.log(data)
      const { data: res } = await this.$http.post('/audioVisualFeast/updateAudioVisualFeast', data)
      console.log(res)
